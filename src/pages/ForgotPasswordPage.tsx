...
export function ForgotPasswordPage() {
 ...
 <Link to="/login" className="inline-flex items-center gap-2 text-muted-foreground font-semibold text-xs uppercase tracking-[0.18em] mb-8 hover:text-primary transition-colors">
 ...
 <p className="text-muted-foreground">Мы отправили ссылку для сброса пароля на <b>{email}</b></p>
 <Button asChild variant="outline" className="w-full h-14 rounded-lg">
 <Link to="/login">Вернуться ко входу</Link>
 </Button>
 </div>
 ) : (
 <>
 <h1 className="text-3xl font-semibold text-foreground mb-2">Забыли пароль?</h1>
 <p className="text-muted-foreground mb-8 font-medium">Введите ваш email, и мы пришлем ссылку для восстановления доступа.</p>

 <form onSubmit={handleSubmit} className="space-y-6">
 <div>
 <label className="text-xs font-semibold text-muted-foreground uppercase tracking-[0.18em] mb-3 block ml-1">Email</label>
