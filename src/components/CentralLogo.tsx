...
export function CentralLogo({ colorMode = "glowing-white", className = "" }: CentralLogoProps) {
  ...
  const loopTransition: Transition | undefined = !shouldReduceMotion && isEntered
    ...
    : undefined;
  const logoFill = colorMode === "emerald-gradient" ? "url(#family-dent-emerald-gradient)" : "#fff";
  const logoShadow = colorMode === "glowing-white" ? "drop-shadow-[0_12px_24px_rgba(0,0,0,0.4)]" : "";
      ...
      >
        <motion.div variants={entranceVariants} initial="hidden" animate="visible" onAnimationComplete={() => setIsEntered(true)} className="relative flex h-full w-full items-center justify-center">
          <motion.div animate={loopAnimate} transition={loopTransition} className="relative z-10 flex h-full w-full items-center justify-center">
            <svg aria-label="Family Dent" data-orbital-rings="true" data-color-mode={colorMode} data-logo-glow={colorMode === "glowing-white" ? "true" : "false"} className={`h-full w-full max-h-[340px] pointer-events-none select-none sm:max-h-[416px] lg:max-h-[494px] xl:max-h-[572px] ${logoShadow}`} viewBox="0 0 27 40" fill="none" xmlns="http://www.w3.org/2000/svg">
