...
function Logo({ className }: { className?: string }) {
 return (
 <svg width="196" height="40" viewBox="0 0 196 40" fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
 ...
 <defs>
 <clipPath id="a">
 <path fill="currentColor" d="M0 0h196v40H0z" />
 </clipPath>
 </defs>
 </svg>
 );
...
export function Header() {
 ...
 if (loading) {
 return (
 <div className="w-10 h-10 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-center overflow-hidden hover:border-primary/30 transition-all">
 ...
 <button className="w-10 h-10 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-center overflow-hidden hover:border-primary/30 transition-all">
 {user.photoURL ? (
 ...
 <DropdownMenuContent align="end" className="w-64 p-2 rounded-2xl border-slate-100 mt-2">
 ...
 <Link to="/profile" className="cursor-pointer w-full flex items-center p-2">
 <User className="mr-3 h-4 w-4 text-primary" />
 <span className="font-bold text-sm text-slate-600">Профиль</span>
 </Link>
 </DropdownMenuItem>
 {isDoctor && (
 <DropdownMenuItem asChild className="rounded-xl">
 <Link to="/doctor/dashboard" className="cursor-pointer w-full flex items-center p-2 text-primary">
 <ClipboardList className="mr-3 h-4 w-4" />
 <span className="font-bold text-sm">Кабинет врача</span>
 </Link>
 </DropdownMenuItem>
 )}
 {!isDoctor && (
 <DropdownMenuItem asChild className="rounded-xl">
 <Link to="/profile/bookings" className="cursor-pointer w-full flex items-center p-2">
 <ClipboardList className="mr-3 h-4 w-4 text-primary" />
 <span className="font-bold text-sm text-slate-600">Мои записи</span>
 </Link>
 </DropdownMenuItem>
 )}
 {isAdmin && (
 ...
 <DropdownMenuSeparator className="bg-slate-100" />
 <DropdownMenuItem asChild className="rounded-xl">
 <Link to="/admin" className="cursor-pointer w-full flex items-center p-2 text-amber-600">
 ...
 <DropdownMenuSeparator className="bg-slate-100" />
 <DropdownMenuItem onClick={handleLogout} className="rounded-xl cursor-pointer text-red-500 focus:text-red-500 p-2">
 ...
 <Button variant="ghost" asChild className="hidden lg:inline-flex text-slate-600 font-bold rounded-2xl h-11 px-6 hover:bg-slate-50 transition-all">
 <Link to="/login">Войти</Link>
 </Button>
 <Button asChild className="hidden lg:inline-flex bg-primary hover:bg-primary/90 text-white font-bold rounded-2xl h-11 px-8 shadow-lg shadow-primary/20 transition-all hover:scale-105 active:scale-95">
 ...
 <span className={cn(
 "nav-link-underline",
 isActive(item.href) ? "nav-link-underline-active" : "nav-link-underline-inactive"
 )} />
 </Link>
 ))}
 </nav>
 ...
 <a href="tel:+992 446 60 66 00" className="header-phone-link">
 <Phone className="header-phone-icon" />
 +992 446 60 66 00
 </a>
 <span className="header-working-hours">Пн - Сб: 7:30 - 19:00</span>
 </div>
 ...
 <a href="tel:+992 446 60 66 00" className="header-mobile-phone-btn">
 ...
 >
 {item.label}
 <ChevronDown className="mobile-nav-chevron" />
 </Link>
 ))}
 {user ? (
 <>
 <Link to="/profile" className="mobile-nav-link group" onClick={() => setMobileMenuOpen(false)}>
 Профиль
 <User className="mobile-nav-chevron" />
 </Link>
 {!isDoctor && (
 <Link to="/profile/bookings" className="mobile-nav-link group" onClick={() => setMobileMenuOpen(false)}>
 Мои записи
 <ClipboardList className="mobile-nav-chevron" />
 </Link>
 )}
 {isDoctor && (
 <Link to="/doctor/dashboard" className="mobile-nav-link group" onClick={() => setMobileMenuOpen(false)}>
 Кабинет врача
 <ClipboardList className="mobile-nav-chevron" />
 </Link>
 )}
 {isAdmin && (
 <Link to="/admin" className="mobile-nav-link group" onClick={() => setMobileMenuOpen(false)}>
 Админ-панель
 <Settings className="mobile-nav-chevron" />
 </Link>
 )}
 </>
 ) : (
 <>
 <Link to="/login" className="mobile-nav-link group" onClick={() => setMobileMenuOpen(false)}>
 Войти
 <User className="mobile-nav-chevron" />
 </Link>
 <Link to="/register" className="mobile-nav-link group" onClick={() => setMobileMenuOpen(false)}>
 Регистрация
 <ChevronDown className="mobile-nav-chevron" />
 </Link>
 </>
 ...
 <div className="mobile-nav-contact-wrapper">
 <p className="mobile-nav-contact-label">Свяжитесь с нами</p>
 <a href="tel:+992 446 60 66 00" className="mobile-nav-contact-phone">
 +992 446 60 66 00
 </a>
